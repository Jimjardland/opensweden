...
exports.add = app => {
  ...
  app.get('/current-events/:data', currentEvents) // for mock
  app.get('/current-events', currentEvents)

  app.get('/get-available-places/:data', getAvailablePlaces) // for mock
  app.get('/get-available-places', getAvailablePlaces)

  app.post('/add-place', addPlace)
  app.post('/publish-place', publishPlace)

  app.get('/get-public-information/:data', getMessages) // for mock
